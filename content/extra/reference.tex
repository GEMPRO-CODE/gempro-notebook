\subsection{Combinatória e Sequências}
\begin{itemize}[leftmargin=*,label={}]
  ...
  \ifextended
  \item \textbf{Lembrete de interpretações de Catalan.}
  \[
    C_n
  \]
  conta sequências balanceadas de parênteses de tamanho $2n$, formas de árvores binárias enraizadas com $n$ nós internos, triangulações de um $(n+2)$-ágono e matchings perfeitos sem cruzamento em $2n$ pontos.
  \fi
  ...
  \item \textbf{Triangulações de polígono convexo.}
  Número de triangulações de um $n$-gono:
  \[
  ...
  \item \textbf{Enumeração de Pólya.}
  Burnside mais decomposição em ciclos para contar colorações sob simetria. Em geral, só vale a pena deixar como lembrete se o time realmente usa.
  \fi
...
\begin{itemize}[leftmargin=*,label={}]
  ...
  \item \textbf{Teorema das três somas de quadrados.}
...
\subsection{Intervalos, Prefixos e Contribuições}
\begin{itemize}[leftmargin=*,label={}]
  \ifextended
...
\item \textbf{DP contando subsequências}
	\texttt{DP[i]} = número de subsequências até a posição \texttt{i}.
	Temos que:
	\[
...
\hline
	1 & \dfrac{1}{1 - x} \\[10pt]
	a^n & \dfrac{1}{1 - ax} \\[10pt]
	n & \dfrac{x}{(1 - x)^2} \\[10pt]
	\binom{n + k}{k} & \dfrac{1}{(1 - x)^{k + 1}} \\[10pt]
	\binom{n - a + k - 1}{k - 1} & \dfrac{x^a}{(1-x)^{k}} \\[10pt]
	\binom{k}{n} & (1 + x)^k \\[10pt]
	C_n & \dfrac{1 - \sqrt{1 - 4x}}{2x} \\[10pt]
	F_n & \dfrac{x}{1 - x - x^2} \\[10pt]
	\dfrac{D_n}{n!} & \dfrac{e^{-x}}{1 - x} \\[10pt]
	\dfrac{1}{n} & \log\dfrac{1}{1-x}
\end{array}
\]
